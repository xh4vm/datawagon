...
async def status_route(
    status: RouteData,
    kafka_producer: KafkaProducer = Depends(Provide[KafkaProducerServiceContainer.kafka_producer])
) -> None:
...
async def map_status_route(
    ...
    with pg_extractor:
        ...
        #     ('SELECT r.geo from content.railways r '
        #      'JOIN content.nodes n ON ST_Intersects(n.location, r.geo) '
        #      'WHERE ST_Contains( '
